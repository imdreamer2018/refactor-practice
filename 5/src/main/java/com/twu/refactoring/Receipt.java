...
public class Receipt {
    ...
    public double getTotalCost() {
        ...
        if(taxi.isAirConditioned()) {
            totalCost += getMinCost(peakTimeMultiple, Math.min(RATE_CHANGE_DISTANCE, totalKms), PRE_RATE_CHANGE_AC_RATE);
            totalCost += getMinCost(peakTimeMultiple, Math.max(0, totalKms - RATE_CHANGE_DISTANCE), POST_RATE_CHANGE_AC_RATE);
        } else {
            totalCost += getMinCost(peakTimeMultiple, Math.min(RATE_CHANGE_DISTANCE, totalKms), PRE_RATE_CHANGE_NON_AC_RATE);
            totalCost += getMinCost(peakTimeMultiple, Math.max(0, totalKms - RATE_CHANGE_DISTANCE), POST_RATE_CHANGE_NON_AC_RATE);
        }

        return totalCost * (1 + SALES_TAX_RATE);
    }

    private double getMinCost(double peakTimeMultiple, int min, int preRateChangeAcRate) {
